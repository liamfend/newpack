...
export const getPropertyPreview =
  (slug, successCallback = () => {}) =>
  dispatch => {
    fetch({
      dispatch,
...
const onSaveSuccess = (dispatch, successCallback) => data => {
  if (data && data.updateProperty && data.updateProperty.property) {
    ...
    if (successCallback) {
      successCallback(data.updateProperty.property)
    }
  }
}
...
export const saveProperty =
  (successCallback, communication = actions.SAVE_PROPERTY_CS) =>
  (dispatch, getState) => {
    ...
    if (hasUnsavedData(editedFields, rooms)) {
      fetch({
        ...
        onError: e => {
          if (e && e[0] && e[0].message && e[0].message.includes('PROPERTY_DRAFT_EXIST')) {
    ...
    onSuccess: data => {
      ...
      if (status === 'FAILED') {
        if (error === 'PROPERTY_CITY_UNPUBLISHED') {
          message.error(
            i18n.t('cms.properties.edit.detail.city.state_invalid', {
              invalid: description,
            }),
          )
        } else {
...
const getEditedRooms = cloneRooms =>
  cloneRooms
    ...
    .map(room => {
      ...
      const returnObj = {
        ...
        links:
          room.node.links &&
          room.node.links.reduce((result, item) => {
            if (item.action) {
              const newLink = {
                ...item,
                _action: item.action,
        ...
        unitTypeBedSizes:
          room.node.unitTypeBedSizes &&
          room.node.unitTypeBedSizes.reduce((result, item) => {
            if (item.action) {
              const newItem = {
          ...
          : room.node.listings
              ...
              .map(listing => {
                ...
                if (
                  !isLandlordRole() &&
                  listing.action &&
                  listing.action !== updateMutation.INSERT
                ) {
                  listingData.id = listing.id
                }
                ...
                if (listing.discountType !== 'NO_DISCOUNT' && listing.discountType !== null) {
                  ...
                  listingData.discountValue = listing.discountValue
                    ? listing.discountValue.toString()
                    : null
                } else {
...
export const updatePropertyListing = (roomId, listingId, record) => (dispatch, getState) => {
  const roomsData = getState().dashboard.propertyEdit.get('cloneRoomListingData').toJS()
  ...
  if (
    !record &&
    typeof targetRoomIndex !== 'undefined' &&
    typeof targetListingIndex !== 'undefined'
  ) {
