...
export const isNetworkIssue = error => error.message && (error.message.indexOf('timeout') !== -1 || error.message.indexOf('Network') !== -1);
...
export const fetch = ({
  ...
  axios({
    ...
    if (typeof response.data.errors === 'undefined' && response.data.data) {
      ...
      if (errorAction) { dispatch(errorAction(response.data.errors)); }
