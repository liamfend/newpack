...
export const getPendingApprovalList = (successCallback = () => {}, params) => (dispatch) => {
  fetch({
    dispatch,
...
export const savePropertyDraft = (successCallback = () => {}) => (dispatch, getState) => {
  const id = getState().dashboard.propertyEdit.getIn(['payload', 'id']);
  const editedFields = getState().dashboard.propertyEdit.get('editedFields').toJS();
  const landlordId = getState().dashboard.propertyEdit.getIn(['payload', 'landlord', 'id']);
  const params = {
  ...
  fetch({
    ...
    onError: (e) => {
      if (
        e && e[0] && e[0].message
        && e[0].message.includes('PROPERTY_DRAFT_STATUS_ERROR')
      ) {
...
export const processPropertyDraft = (params, successCallback = () => {}) => (dispatch) => {
  fetch({
    dispatch,
    endpoint: endpoints.defaultUrl.url(),
...
export const cancelPropertyDraft = (successCallback = () => {}, id) => (dispatch) => {
  fetch({
    dispatch,
    endpoint: endpoints.defaultUrl.url(),
...
export const initProperty = draft => (dispatch, getState) => {
  ...
  const updatedRoomsData =
    mergeUnitTypes(property.unitTypes, rooms, draftLinks, draftGallery, true);
...
export const rejectPropertyDraft = (successCallback = () => {}, params) => (dispatch) => {
  fetch({
    dispatch,
    endpoint: endpoints.defaultUrl.url(),
...
(successCallback = () => {}, params) => (dispatch, getState) => {
  const editedFields = getState().dashboard.propertyEdit.get('editedFields').toJS();
  const property = getState().dashboard.propertyEdit.get('payload').toJS();
  const data = {
...
export const getPropertyComments = (slug, successCallback = () => {}) => (dispatch) => {
  fetch({
    dispatch,
...
export const expirePropertyDraft = (draftIds, successCallback = () => {}) => (dispatch) => {
  fetch({
    dispatch,
