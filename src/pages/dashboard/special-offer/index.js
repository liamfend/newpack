...
import {
  ...
  offerTypes,
  locales,
  platformEntity, entityAction } from '~constants';
...
export default class SpecialOffer extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    this.operationChecker(
      nextProps,
      'list',
      this.props.t('cms.message.error'),
    );
    this.operationChecker(
      nextProps,
      'create',
      this.props.t('cms.message.offers.created_success'),
    );
    this.operationChecker(
      nextProps,
      'update',
      this.props.t('cms.message.offers.updated_success'),
    );
    this.operationChecker(
      nextProps,
      'delete',
      this.props.t('cms.message.offers.deleted_success'),
    );
  }
  ...
  operationChecker = (nextProps, type, messages, processStep = true) => {
    if (type === 'list') {
      if (
        ...
        nextProps.communication.list.status !==
          this.props.communication.list.status
      ) {
    ...
    if (
      this.props.communication[type].status === communicationStatus.FETCHING
    ) {
      if (nextProps.communication[type].status === communicationStatus.IDLE) {
        ...
        nextProps.communication[type].status !== communicationStatus.FETCHING
      ) {
        if (
          nextProps.communication[type].errorInfo.errorDescription &&
          nextProps.communication[type].errorInfo.errorDescription.indexOf(
            'already has',
          ) !== -1
        ) {
          message.error(
  ...
  formatData = () => {
    ...
    this.props.list.payload.forEach((listItem, index) => {
      ...
      listItem.unitTypes.forEach((unitType) => {
        const isHaveProperty = allProperties.find(
          property => unitType.property.id === property.id);
        if (!isHaveProperty) {
        ...
        listItem.unitTypes.forEach((unitType) => {
          if (unitType.property.id === property.id) {
            if (allProperties[propertyIndex].unitType) {
              allProperties = update(allProperties, { [propertyIndex]: { unitType:
                { $push: [unitType] } } });
              allProperties = update(allProperties, { [propertyIndex]: { unitType:
                { listings: { $set: [] } } } });
            } else {
  ...
  formatFilters = filters => ({
    pageNumber:
      Number(filters.pageNumber) || this.defaultSearchParams.pageNumber,
  ...
  tableColumns = () => [
    ...
    },
    {
      ...
      filterIcon: (
        <Icon
          type="filter"
          style={ { color: this.state.filters.title ? '#38b2a6' : '#c8c9cb' } }
        />
      ),
    ...
    },
    {
      ...
      filterDropdown: (
        <TableColumnSearch
          searchType="property"
          ref={ (component) => { this.columnInputs.propertyFilter = component; } }
          valueData={ this.state.filters.propertyId || '' }
          placeholder={ this.props.t('cms.table.column.search.property_name') }
          onSearch={ (id) => { this.handleColumnFilter('propertyId', id); } }
          t={ this.props.t }
        />
      ),
      filterIcon: (
      ...
      render: (text) => {
        ...
        const unitTypes = types =>
          types.map(type => (<div className="special-offer__property-item" key={ type.name }>
            {type.name}
            {type.listings.length > 0 && ` (${type.listings.length} ${this.props.t('cms.special_offer.form.listings')})`}
          </div>));
        ...
        return (
          ...
          >
            ...
            <If condition={ text.length > 1 }>
              <span className="special-offer__right-text">{`${text.length} ${this.props.t('cms.table.column.offer.properties')}`}</span>
            </If>
          </Tooltip>
        );
      },
    },
    {
      ...
      render: (text, record) => {
        if (record.properties) {
          let landlord = record.properties.map(property =>
    ...
    },
    {
      ...
      filterDropdown: (
        <TableColumnSearch
          ...
          onSearch={ (id) => { this.handleColumnFilter('cityId', id); } }
          t={ this.props.t }
      ...
      filterIcon: (
        <Icon
          type="filter"
          style={ { color: this.state.filters.cityId ? '#38b2a6' : '#c8c9cb' } }
        />
      ),
    ...
    },
    {
      ...
      render: (text) => {
        if (text) {
          if (text === 'INTERNAL') {
            return <span>{this.props.t('cms.special_offer.column.owner_type.filter.internal')}</span>;
          }
            ...
            >
              <div className="table-header__button-container">
                <Button onClick={ () => this.opreatOffer('add', {}) } >
                  <Icon type="plus" />
                  {this.props.t(
                    'cms.table.header.button.add_new_special_offer',
                  )}
                </Button>
              </div>
            </If>
