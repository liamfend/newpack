...
export default class RoomListingReviewItem extends React.Component {
  constructor(props) {
    ...
    this.listColumns = [
      ...
      {
        title: `${props.t('cms.properties.edit.review_modal.room.price')}(${this.props.t(`cms.listing.billing_cycle.per_${
  ...
  renderTenancy = (text, record) => {
    ...
    return (<div className={ classNames('room-review__tenancy', {
      'room-review__deleted': this.props.isChangedVersion && record.action === updateMutation.DELETE,
    }) }
    >
      ...
      <If condition={ this.props.isChangedVersion && record.action === updateMutation.INSERT }>
        <span className="room-review__new-label">{this.props.t('cms.properties.edit.review_modal.new-label')}</span>
      </If>
    </div>
    );
  }
  ...
  renderStay = (text, record) => {
    ...
    return (<span className={ classNames('room-review__listing__stay') }>
      {tenancy.stay || '--'}
    </span>);
  }
  ...
  renderAvailability = (text, record) => (
    <div>
      <If condition={ record.type !== 'PLACEHOLDER' }>
        <span className={ classNames('room-review__listing__availability',
        ...
        >
          {this.props.t(`cms.table.value.availability.${record.availability && record.availability.toLowerCase()}`)}
        </span>
      </If>
      <If condition={ record.type === 'PLACEHOLDER' }>
          ...
          <Choose>
            ...
            <Otherwise >
              <For index="index" each="field" of={ topic.value }>
                <div
                  className={ classNames('room-review__detail__field', { 'room-review__detail__field--line': field.key === 'unitTypeBedSizes' }) }
                  key={ index }
                >
                  <div className="room-review__detail__field-name">
                    {field.fieldName}
                  </div>
  ...
  render() {
    ...
    return (
      <div
        className="room-review__item"
      >
        <div
          className="room-review__item__room"
          ref={ (roomItem) => { this.roomRef = roomItem; } }
        >
          ...
          <If condition={ isChangedVersion && room.action === updateMutation.INSERT }>
            <span className="room-review__new-room-label">{t('cms.properties.edit.review_modal.new-label')}</span>
          </If>
          <span className={ classNames('room-review__item__name', { 'room-review__deleted': this.props.isChangedVersion && room.action === updateMutation.DELETE }) } >{room.name}</span>
          <span className="room-review__item__category">{room.category ? t(`cms.table.value.category.${room.category.toLowerCase()}`) : ''}</span>
          <If condition={ !(isChangedVersion && room.action === updateMutation.DELETE) }>
            <span
              ...
              role="presentation"
            >
              {t(`cms.properties.edit.review_modal.room.${showDetail ? 'hide_detail' : 'more_detail'}`)}
            </span>
          </If>
        </div>
        <If condition={ showListing && room.listings && room.listings.length > 0 }>
          <div className="room-review__listing">
            <Table
              ...
              dataSource={ isChangedVersion ?
                room.listings.filter(item => item.action) : room.listings }
              rowClassName={ record =>
                (isChangedVersion && record.action === updateMutation.DELETE ? 'room-review__delete-row' : '') }
              pagination={ false }
              rowKey="id"
            />
          </div>
        ...
        <If condition={ showDetail }>
          <div
            ...
            ref={ (roomDetail) => { this.detailRef = roomDetail; } }
          >
            { this.renderDetail() }
          </div>
        </If>
