...
export default class UploadingModal extends React.Component {
  ...
  uploadAll = callback => {
    const stillInProgress = this.state.list.images.find(v => v.status === uploadStatus.IN_PROGRESS)
  ...
  uploadItem = item => {
    this.updateStatus(item, uploadStatus.IN_PROGRESS, 0)
    ...
    const config = {
      ...
      onUploadProgress: progressEvent => {
        ...
        this.updateStatus(item, uploadStatus.IN_PROGRESS, progress)
      },
      headers,
    ...
    axios
      ...
      .then(response => {
        ...
        this.updateStatus(itemToUpdate, uploadStatus.SUCCESS)
      })
      .catch(() => {
  ...
  updateStatus = (item, status, progress = 100) => {
    ...
    if (differentStatus) {
      const stillInProgress = this.state.list.images.find(
        v => v.status === uploadStatus.IN_PROGRESS,
      )
      const stillVideoInProgress = this.state.list.videos.find(
        v =>
          v.status === uploadStatus.IN_PROGRESS ||
  ...
  onDelete = item => {
    this.onCancel(item)
    this.state.list.images.splice(this.state.list.images.indexOf(item), 1)
    this.forceUpdate()
  ...
  onConfirm = () => {
    ...
    const imageSuccessList = this.state.list.images.filter(
      item => item.status === uploadStatus.SUCCESS,
    )
    const videoSuccessList = this.state.list.videos.filter(
      item => item.status === uploadStatus.SUCCESS,
    )
    ...
    if (
      ...
      setTimeout(() => {
        ...
        onConfirm(
          librarySelected,
          allSuccess.map(item => item.response),
          isClose,
        )
      }, 900)
    }
        ...
        {outerProps => (
          ...
          >
            <div className="gallery-uploading__header">
              ...
              <button className="gallery-uploading__shrink" onClick={this.handleShrink}>
            ...
            >
              ...
              <div className="gallery-uploading__images">
                ...
                <Dropzone {...this.dropProps({ noDrag: true })}>
                  {innerProps => (
                    <div className="gallery-uploading__item-upload" {...innerProps.getRootProps()}>
            ...
            <div className="gallery-uploading__footer">
              <span className="gallery-uploading__footer-status" {...this.renderSummary(summary)} />
              <div className="gallery-uploading__footer-buttons">
                <Choose>
                  <When condition={imageUploading === true || videoUploading === true}>
                    <Choose>
                      ...
                      <Otherwise>
                        <Button className="gallery-uploading__button" onClick={this.stopUploadAll}>
                          {t('cms.properties.edit.gallery.uploading.button.stop_upload')}
                        </Button>
                      </Otherwise>
                    </Choose>
                ...
                >
                  <Choose>
                    <When
                      condition={
                        !imageUploading && !videoUploading && summary.success > 0 && !disableConfirm
                      }
                    >
                      <Button
