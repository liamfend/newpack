  ...
  getCoverPhoto = () => {
    if (this.props.items && this.props.items.length > 0) {
      this.props.items.some((item) => {
        if (item && item.contentType && getFileType(item.contentType) === 'image' && item.imageHash) {
                ...
                <When condition={ media.multiple }>
                  <div
                    className={ classNames(
                      ...
                      {
                        'gallery-library__dropzone--upload-active': drapStatus === SortableLibrary.DRAP_STATUS_ENTER,
                      },
                    ) }
