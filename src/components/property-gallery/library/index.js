...
export default class Library extends React.Component {
  ...
  onConfirmDelete = () => {
    this.setState({
      ...
      this.props.onDelete(
        this.props.id,
        this.container.getSelectedIndexes(),
      );
    });
  }
  ...
  createHelper = (element) => {
    ...
    let imgHTML;
    if (type === 'video') {
      imgHTML = htmlToString(element.node.querySelector('.photo-item__img-video-success,.photo-item__img-video-compress'));
    } else {
      imgHTML = `<img src=${imageUrl(element.getItem(), imageSizes.small)} class="photo-item__img-multiple" />`;
      if (element.node.querySelector('.photo-item__cover-photo')) {
        imgHTML += `<div class="photo-item__cover-photo">
          <span class="photo-item__cover-text">${this.props.t('cms.properties.edit.gallery.cover_text')}</span>
        </div>`;
      }
    }
  ...
  haveValidListing = (listings) => {
    if (listings && listings.length > 0) {
      if (
        listings.filter(
          listing =>
            listing.action === updateMutation.DELETE
            || listing.action === updateMutation.NEW,
        ).length
        === listings.length
      ) {
        return true;
              ...
              <div className="gallery-library__action">
                <If condition={ !description }>
                  <h4
                    ...
                    ref={ (node) => { this.vrTooltipContainer = node; } }
                  >
                    {/* <If condition={
