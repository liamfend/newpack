  ...
  componentDidMount() {
    ...
    links.map((link) => {
      ...
      if (section === 'Property') {
        ...
        field[`combinedSearchComponent-${link.id}`] = rooms.map(item => item.node)
          .filter((item) => {
            const unitTypeIds = link.unitTypeIdToLinkId
              .map(unitTypeIdToLinkIdItem => unitTypeIdToLinkIdItem.unitTypeId);
      ...
      <div className="virtual-tour-link-content">
        ...
        <div>
          <For of={ links.filter(link => link.action !== 'DELETE') } each="virtualTourLinkItem">
            <div className="virtual-tour-link-item" ref={ (node) => { this.selectContainer = node; } } key={ virtualTourLinkItem.id }>
              <Row gutter={ 20 }>
                ...
                <Col span={ section === 'Overall' ? 17 : 9 }>
                  <Form.Item>
                    <Tooltip
                      ...
                      overlayClassName={
                        classNames('vr-tooltip', { 'vr-tooltip--show': getFieldValue(`enabled-${virtualTourLinkItem.id}`) })
                      }
                      placement="top"
                    ...
                    >
                      {
                        getFieldDecorator(`link-${virtualTourLinkItem.id}`, {
                          ...
                          rules: [
                            ...
                            },
                            {
                              validator: (rule, value, callback) => {
                                links.filter(link => link.id !== virtualTourLinkItem.id)
                                  .map((link) => {
                              ...
                              message: this.props.t('cms.properties.edit.gallery.duplicated_virtual_tour_link_error'),
                            },
                          ],
                          ...
                          <Input
                            ...
                            className={
                              classNames('virtual-tour-link-item__middle url', { 'url--valid': this.validatUrl(getFieldValue(`link-${virtualTourLinkItem.id}`)) })
                            }
                            autoComplete="off"
                            placeholder={ t('cms.properties.edit.gallery.virtual_tour.link_address_hint') }
                            addonBefore={ <span className="virtual-tour-link-item__url-prefix">{ vrUrlPrefix }</span> }
                            addonAfter={
                              <Icon
                                type="arrow-right"
                                onClick={ () => { this.handleCheckButtonClick(getFieldValue(`link-${virtualTourLinkItem.id}`)); } }
                              />
                            }
                          />,
                        )
                ...
                <Col span={ 4 }>
                  <Form.Item style={ { width: '100%' } }>
                    <Tooltip
                      ...
                      getPopupContainer={ () => this.selectContainer }
                      overlayClassName={
                        classNames('vr-tooltip', { 'vr-tooltip--show': getFieldValue(`enabled-${virtualTourLinkItem.id}`) || isLandlordRole() })
                      }
                      placement="top"
                      title={ t(isLandlordRole() ?
                      ...
                      <div>
                        {
                          getFieldDecorator(`displayRegion-${virtualTourLinkItem.id}`, {
                            ...
                            <Select
                              style={ { width: '100%' } }
                              placeholder={ t('cms.properties.edit.gallery.virtual_tour.select_locale') }
                              getPopupContainer={ () => this.selectContainer }
                              disabled={ getFieldValue(`enabled-${virtualTourLinkItem.id}`) || isLandlordRole() }
                            >
                              <Select.Option value="ALL">{t('cms.properties.edit.address.display_permission_ALL')}</Select.Option>
                              <Select.Option value="ROW">{t('cms.properties.edit.address.display_permission_ROW')}</Select.Option>
                              <Select.Option value="CN">{t('cms.properties.edit.address.display_permission_CN')}</Select.Option>
                            </Select>,
                          )
                        }
                      </div>
                        ...
                        <div>
                          {
                            getFieldDecorator(`area-${virtualTourLinkItem.id}`, {
                              ...
                              <Select
                                ...
                                disabled={ getFieldValue(`enabled-${virtualTourLinkItem.id}`) }
                              >
                                <Select.Option value="GENERAL">{t('cms.properties.edit.gallery.library.title.general')}</Select.Option>
                                <Select.Option value="BUILDING_EXTERIOR">{t('cms.properties.edit.gallery.library.title.building_exterior')}</Select.Option>
                                <Select.Option value="COMMON_INDOOR_SPACES">{t('cms.properties.edit.gallery.library.title.common_indoor_spaces')}</Select.Option>
                                <Select.Option value="COMMON_OUTDOOR_SPACES">{t('cms.properties.edit.gallery.library.title.common_outdoor_spaces')}</Select.Option>
                              </Select>,
                            )
                          }
                        </div>
                ...
                <Col className="virtual-tour-link-item__middle" span={ 2 }>
                  ...
                  <Tooltip
                    ...
                    getPopupContainer={ () => this.selectContainer }
                    overlayClassName={
                      classNames('vr-tooltip', { 'vr-tooltip--show': !this.checkSwitchEnabled(section, virtualTourLinkItem.id, links) })
                    }
                    placement="topRight"
                    title={ this.state.isSameLocale ? t('cms.properties.edit.gallery.same_overall_virtual_tour_link') :
                      t('cms.properties.edit.gallery.virtual_tour.disable_switch_hint') }
                  >
                    <Col span={ 24 } className="virtual-tour-link-item__middle" style={ { padding: 0, justifyContent: 'flex-end' } }>
                      {
                        getFieldDecorator(`enabled-${virtualTourLinkItem.id}`)(
                          <Switch
                            defaultChecked={ virtualTourLinkItem.enabled }
                            disabled={ !this.checkSwitchEnabled(section,
                              virtualTourLinkItem.id, links) }
        ...
        <If condition={ section !== 'Overall' || (section === 'Overall' && links.length < 2) }>
          <div className="btn__add-another-link" onClick={ () => { this.props.onAddAnOtherLink(section); } } role="presentation">
