...
class VirtualTourLinkModal extends React.Component {
    ...
    switch (section) {
      case 'Overall': {
        return (
          property.links &&
          property.links
            .filter(link => link.label === 'OVERALL')
            .map(link => {
              const newLink = { ...link }
      ...
      case 'Property': {
        return (
          property.links &&
          property.links
            .filter(link => link.label === 'PROPERTY')
            .map(link => {
              const newLink = { ...link }
              const action = '_action'
              newLink.action = 'UPDATE'
    ...
    switch (section) {
      ...
      case 'Room': {
        ...
        roomLinks.map(roomLink => {
          const room = { ...roomLink }
          if (roomLink.unitTypeIdToLinkId && roomLink.unitTypeIdToLinkId.length) {
            room.unitTypeIdToLinkId = roomLink.unitTypeIdToLinkId.filter(
  ...
  convertData = (apiLinks, formValuesObj) => {
    let newLinks = apiLinks.map(link => {
      ...
      if (newLink.unitTypeIdToLinkId && newLink.unitTypeIdToLinkId.length) {
  ...
  handleConfirm = () => {
    ...
    form.validateFieldsAndScroll({ scroll: { offsetTop: 76, offsetBottom: 76 } }, err => {
      if (!err) {
        this.props.onCancel()
  ...
  handleSetSelectedRooms = (currentLink, selectedRooms) => {
    ...
    const newUnitTypeIdToLinkId = currentLink.unitTypeIdToLinkId.map(
      (unitTypeIdToLinkIdItem, index) => {
  ...
  render() {
    ...
    return (
      <Modal
        ...
        footer={
          <Button onClick={this.handleConfirm} className="btn-confirm" type="primary">
              ...
              <For of={this.sections} each="section">
                ...
                >
                  <div className="section-title">
                    {t('cms.properties.edit.gallery.section_virtual_tour_link', { section })}
                  </div>
                  <If condition={section === 'Overall'}>
                    <div className="overall-virtual-tour-link__reminder">
                      <Icon
                        ...
                        className="overall-virtual-tour-link__reminder__icon"
                      />
                  ...
                  <Choose>
                    <When
                      condition={
                        ...
                        this.getDifferentSectionLinks(section).filter(
                          link => link.action !== 'DELETE',
                        ).length
                      }
                    >
                      <VirtualTourLinkContent
                        links={this.getDifferentSectionLinks(section).filter(
                          link => link.action !== 'DELETE',
                        )}
                        form={form}
                        t={t}
                        ...
                        onAddAnOtherLink={section => {
                          this.handleAddAnotherLink(section)
                        }}
                        onDeleteLink={(section, id) => {
                          this.handleDeleteLink(section, id)
                        }}
