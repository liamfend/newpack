...
export default class CombinedSearchComponent extends React.Component {
  ...
  render() {
    ...
    return (
      <div className="combined-search-component" ref={ (node) => { this.searchComponentContainer = node; } }>
        <Form.Item>
          {
            getFieldDecorator(`combinedSearchComponent-${virtualTourLink.id}`, {
              ...
              <Input
                ...
                onFocus={ () => { this.setState({ searchComponentVisible: true }); } }
                disabled={ disabled }
          ...
          <If condition={ this.state.searchComponentVisible }>
            <SearchComponent
              ...
              onChange={ (value) => {
                const field = {};
                field[`combinedSearchComponent-${virtualTourLink.id}`] = value.value.map(item => item.name).join('; ');
