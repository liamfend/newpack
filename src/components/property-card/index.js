...
const imageHost =
  getEnvironment() === environments.PROD ? '//image.student.com' : '//image.dandythrust.com'
      ...
      >
        ...
        >
          <div className="property-card__container">
            ...
            <div className="property-card__content-container">
              <p className="property-card__name-container">
                ...
                <If condition={get(property, 'averageRating') > 0}>
                  <span className="property-card__average-rating">{reviewRating}</span>
                </If>
              </p>
            ...
            <div
              className={classNames('property-card__status-label', {
                'property-card__status-label--grey':
                  get(property, 'status') === propertyStatus.UNPUBLISHED,
              })}
            >
              {t(`cms.property_card.status.tag.${toLower(get(property, 'status'))}`)}
            </div>
          </div>
        </Skeleton>
      </Link>
