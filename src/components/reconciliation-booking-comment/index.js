...
export default class ReconciliationBookingComment extends React.Component {
  ...
  render() {
    const { opportunityStateChangesNote, t, bookingStatus, isActiveOpportunity } = this.props

    const caseNumber =
      opportunityStateChangesNote.opportunityCase &&
    ...
    return (
      <div className="reconciliation-booking-comment">
        ...
        <If condition={bookingStatus === bookingLandlordStatus.PENDING_APPROVAL}>
          <div
            className={`reconciliation-booking-comment__tag-container ${
              isActiveOpportunity ? '' : 'reconciliation-booking-comment__tag-container--blue'
            }`}
          >
            ...
            <If condition={isActiveOpportunity && caseNumber}>
              ...
              <span className="reconciliation-booking-comment__tag-sign">{caseNumber}</span>
            </If>
          </div>
        </If>
        ...
        <div className="reconciliation-booking-comment__content-container">
          ...
          <ul className="reconciliation-booking-comment__gallery-container">
            <For
              each="imageItem"
              of={opportunityStateChangesNote.opportunityPendingNoteFiles || []}
              index="index"
            >
              <li
                className="reconciliation-booking-comment__gallery-item"
                onClick={() => {
                  this.handleClickOpenGallery(index)
                }}
                role="presentation"
                key={index}
              >
