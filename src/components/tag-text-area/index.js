...
export default class TagTextArea extends React.Component {
  ...
  addTag = () => {
    ...
    if (
      this.state.list.indexOf(this.state.value) === -1 &&
  ...
  showLabel = name =>
    name.length > this.props.tagLengthLimit
      ? `${name.slice(0, this.props.tagLengthLimit)}...`
      : name
    ...
    return (
      <div>
        <div
          ...
          ref={node => {
            this.tagTextAreaContainer = node
          }}
        >
        ...
        <div>
          <If condition={this.state.list.length > countLimit || this.state.showErr}>
            <span className="tag-text-area__error-msg">
              <Choose>
                <When condition={this.state.showErr}>{this.props.errorMessage}</When>
                <Otherwise>
                  {this.props.t('cms.properties.edit.list.other.length_limit_error')}
                </Otherwise>
              </Choose>
            </span>
          </If>
          <span
            className={classNames('tag-text-area__counter', {
