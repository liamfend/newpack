...
export default class PropertyReviewModal extends React.Component {
  constructor(props) {
    ...
    this.showSwitchLink = props.showChangedVersion
              ...
              <Otherwise>
                <For each="field" of={ fields } index="index">
                  <If condition={
                    !(isLandlordRole()
                    && (
                      (name === 'details' && ['headlineCn', 'descriptionCn', 'rankType', 'rank'].indexOf(field.key) !== -1)
                  ...
                  >
                    <div key={ index } className="property-review__item">
                      <div className="property-review__item__name">
                        {t(field.name)}
                      </div>
                      <div className="property-review__item__content">
                        <Choose>
                          <When condition={
                            richTextFields.indexOf(field.key) !== -1
                            && field.text
                          }
                          >
                          ...
                          <Otherwise>
                            <span dangerouslySetInnerHTML={ {
                              __html: field.text === null || field.text === undefined || field.text === '' ? '-' : field.text } }
                            />
                          </Otherwise>
                        </Choose>
                      </div>
  ...
  renderRoomListing = () => {
    ...
    return (
      <If condition={ !this.state.isChangedVersion || isRoomChanged(roomData) }>
        <div className="property-review__topic">
          {this.renderTopicName('room-listing', this.isRoomDataEmpty(roomData) ? 'rooms' : 'listings')}
          <RoomListingReview
            t={ t }
            isChangedVersion={ this.state.isChangedVersion }
  ...
  render() {
    ...
    return (
      <div
        className={ classNames('property-review', { 'property-review--collasped': this.isCollasped() }) }
      >
        <div className="property-review__header">
          {t(`cms.properties.edit.review_modal.${isChangedVersion ? 'changed_version_title' : 'full_version_title'}`)}
          <If condition={ this.showSwitchLink }>
            ...
            >
              <Choose>
                ...
                <Otherwise>
                  {t('cms.properties.edit.review_modal.change_to_compare')}
                </Otherwise>
              </Choose>
            </a>
          </If>
        ...
        <div className="property-review__footer">
          <For each="button" of={ buttons } index="key">
            <Button
              key={ key }
              className={ classNames('property-review__btn', { 'property-review__btn--not-primary': !button.primary }) }
