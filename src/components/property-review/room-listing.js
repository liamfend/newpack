  ...
  getBeds = value => {
    ...
    return value
      ...
      .map(item => {
        ...
        return `${this.props.t(`cms.properties.edit.rooms.bed_size.${item.type.toLowerCase()}`)}${
          item.width && item.length ? `: ${item.width}x${item.length}` : ''
        }${item.bedCount ? ` (${item.bedCount})` : ''}`
      })
      .join('、')
  }

  getCategory = value =>
    value ? this.props.t(`cms.properties.edit.rooms.category.${value.toLowerCase()}`) : ''

  getRoomSize = (value, room) =>
    value && room.roomType
      ? `${value} ${this.props.t(
          `cms.properties.edit.rooms.room_size.${room.roomType.toLowerCase()}`,
        )}`
      : ''
    ...
    Object.keys(this.roomFormatOption).map(topicName => {
      ...
      if (topicName === 'facilities') {
        ...
        Object.keys(topicOption).map(fieldName => {
          ...
          const fieldValue = fieldOption.getValue
            ? fieldOption.getValue(room[fieldName], room)
            : room[fieldName]
          topic.push({
            key: fieldName,
  ...
  removeUnchangedInDetail = (originalRoom, updatedRoom) => {
    ...
    updatedRoom.roomDetails.forEach((topicItem, topicIndex) => {
      ...
      if (updatedTopic.name === 'facilities') {
        topic = originalTopic.value === updatedTopic.value ? null : updatedTopic.value
      } else {
