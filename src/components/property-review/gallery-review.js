...
export default class GalleryReview extends React.Component {
  constructor(props) {
    ...
    this.coverPhotoId = 0;
    this.originList = formatList(formatLibraries(
      props.originalData.unitTypes.edges,
      props.originalData.links,
    ),
    props.originalData);
  }
  ...
  handleGalleryCategoryChanged() {
    Object.keys(this.props.changedData.data.value.list).map((key) => {
      ...
      if (!this.originList[key] || (
        this.props.changedData.data.value.list[key].length !== this.originList[key].length ||
        (this.checkCategoryChanged(
          this.originList[key],
          this.props.changedData.data.value.list[key],
        )))) {
  ...
  render() {
    ...
    let galleryTypes = isChangedVersion ?
      changedCategory.map(category => (category === 'property:room' ? 'room' : category.split(':')[0])) :
          ...
          <For index="galleryIndex" each="galleryType" of={ galleryTypes }>
            <div className="gallery-review__container" key={ galleryIndex }>
              ...
              <For
                index="index"
                each="photoLibrary"
                of={ isChangedVersion ? galleriesSorted[galleryType].filter(category =>
                  changedCategory.indexOf(category.id) !== -1,
                ) : galleriesSorted[galleryType] }
              >
                ...
                >
                  <div key={ index }>
                    <div className={ classNames('gallery-review__sub-title', {
                    ...
                    <For index="imgIndex" each="imgItem" of={ photoLibrary.list }>
                      <If
                        condition={
                          imgItem.transcodedStatus !== gallery.videoStatus.error
                        }
                      >

                        <div
                          key={ imgIndex }
                          ref={ (e) => { this[`img${galleryIndex}${index}${imgIndex}`] = e; } }
                          onMouseOver={ () => {
                            this.showImageModal(galleryIndex, index,
                              imgIndex, imgItem);
                          } }
                        ...
                        >
                          <Choose>
                            <When condition={ photoLibrary.multiple }>
                              <div className="gallery-review__img-container--multiple" data={ imgIndex.id }>
                                <Choose>
                                  <When condition={ imgItem.contentType && getFileType(imgItem.contentType) === 'video' }>
                                    <Choose>
                                      <When condition={
                                      ...
                                      >
                                        <Tooltip
                                          getPopupContainer={ () => this[`img${galleryIndex}${index}${imgIndex}`] }
                                          placement={ 'top' }
                                        ...
                                        >
                                          <div className="photo-item__img-video-success">
                                            ...
                                            <img
                                              ...
                                              src={ imgItem && imgItem.links ? imgItem.links.thumbnail : '' }
                                            />
                                          </div>
                                        </Tooltip>
                                      </Otherwise>
                            ...
                            <Otherwise>
                              <div className={ 'gallery-review__img-container--single' }>
                                <img src={ imageUrl(imgItem, imageSizes.middle) } className={ 'gallery-review__img-item--single' } alt={ imgItem.filename } title={ imgItem.filename } />
                              </div>
                            </Otherwise>
                          </Choose>
                        </div>
        ...
        >
          <div className="gallery-review__container">
            ...
            <For of={ this.sections } each="section">
              <If
                condition={ this.formatVirtualLinks(section, rooms.map(room => room.node)).length }
              >
                <div className="gallery-review__sub-title">
                  <div>{this.props.t('cms.properties.edit.gallery.section_virtual_tour_link', { section })}</div>
                </div>
                <For of={ this.formatVirtualLinks(section, rooms.map(room => room.node)) } each="link">
                  <div
                    className={ classNames(
                      'gallery-review__container__virtual-link', {
                    ...
                    <If condition={ link.action !== 'DELETE' } >
                      ...
                      <If condition={ section === 'Property' }>
                        <Row type="flex" gutter={ 34 } style={ { flexWrap: 'nowrap' } }>
                          <Col span={ 5 }>
                            <div className="gallery-review__container__virtual-link__title">
                              {this.props.t('cms.properties.edit.gallery.virtual_tour.area_of_property')}
                            </div>
                          </Col>
                          <Col span={ 19 }>
                            <div className="gallery-review__container__virtual-link__value">
                              { this.props.t(`cms.properties.edit.gallery.library.title.${link.area.toLowerCase()}`) }
                            </div>
                          </Col>
                        </Row>
                      </If>
                      <If condition={ section === 'Room' }>
                        <Row type="flex" gutter={ 34 } style={ { flexWrap: 'nowrap' } }>
                          ...
                          <Col span={ 19 }>
                            <div className="gallery-review__container__virtual-link__value room-section" ref={ (node) => { this.tooltipContainer = node; } }>
                              <span
                                className="room-value"
                                // eslint-disable-next-line react/no-danger
                                dangerouslySetInnerHTML={ {
                                  __html: this.getRoomsName(rooms.map(room => room.node),
                                    link.unitTypeIdToLinkId.map(item => item.unitTypeId)).replace(/\|/g, '<span class="vertical-line">|</span>'),
                                } }
                              />
                            </div>
                          </Col>
