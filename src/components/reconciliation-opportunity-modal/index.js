...
const ReconciliationOpportunityModal = ({ handleClickClose, onConfirm, form, editOpportunity }) => {
  ...
  useEffect(() => {
    form.setFieldsValue({
      finalMoveInDate: editOpportunity.finalMoveInDate ? moment(editOpportunity.finalMoveInDate, 'YYYY-MM-DD') : null,
      finalMoveOutDate: editOpportunity.finalMoveOutDate ? moment(editOpportunity.finalMoveOutDate, 'YYYY-MM-DD') : null,
      finalPrice: editOpportunity.finalPrice,
    });
  }, []);

  const disabledDate = useCallback(current => current < moment(form.getFieldValue('finalMoveInDate')).add(1, 'days'), [form.getFieldValue('finalMoveInDate')]);
  ...
  return (
    <div className="reconciliation-opportunity-modal">
      <div className="reconciliation-opportunity-modal__container">
        ...
        <Form
          layout="vertical"
          className="reconciliation-opportunity-modal__content"
        >
          <Form.Item
          ...
          >
            <div ref={ finalMoveInDateRef } >
              { getFieldDecorator('finalMoveInDate', {
                ...
                rules: [
                  { validator: (rule, value, callback) => {
                    if (!value && !form.getFieldValue('finalMoveInDate')) {
                      callback(t('cms.reconciliation.booking.details.edit.modal.datepicker.empty'));
                    } else {
                      callback();
                    }
                  } },
                ...
                <DatePicker
                  placeholder={ t('cms.reconciliation.booking.details.edit.modal.datepicker.placeholder') }
          ...
          >
            { getFieldDecorator('finalPrice', {
              ...
              <InputNumber
                formatter={ value => `${getSymbolFromCurrency(editOpportunity.currency)} ${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',') }
