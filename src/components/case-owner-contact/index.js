...
export default function CaseOwnerContact(props) {
  ...
  const ownByUserData =
    opportunityCase && opportunityCase.ownByUser ? opportunityCase.ownByUser : ''
  ...
  const contactArray = [
    ...
    },
    {
      title: 'case_update',
      content:
        opportunityCase && opportunityCase.updatedAt
        ...
        <For each="ownerItem" of={contactArray} index="index">
          <li className="case-owner-contact__item" key={ownerItem.title}>
            <span className="case-owner-contact__title">
              {i18next.t(
                `cms.reconciliation.booking.details.cards.label.case_owner.${ownerItem.title}.label`,
              )}
            </span>
            <span className="case-owner-contact__content">{ownerItem.content || '-'}</span>
          </li>
        </For>
      </ul>
    </div>
