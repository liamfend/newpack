...
const MyMap = compose(
  withProps(props => ({
    ...
    googleMapURL:
      'https://maps.googleapis.com/maps/api/js?key=AIzaSyAo3z-iOKf-Mto5TsHcsjXylWdxCt7zZDk&v=3.exp&libraries=geometry,drawing,places&language=en-GB&region=GB',
    containerElement: <div style={{ position: 'relative' }} />,
    mapElement: (
      <div
        style={{ height: props.mapHeight, border: '#c8c9cb solid 1px', backgroundColor: '#c8c9cb' }}
      />
    ),
  })),
    ...
    componentWillMount() {
      ...
      this.setState({
        ...
        resetPin: defaultCenter => {
          ...
          this.props.onChange({
            action: 'reset',
            value: { lat: defaultCenter.lat, lng: defaultCenter.lng },
          })
        },
        onChangeEnd: () => {
          const currentLat = refs.map.getCenter() ? refs.map.getCenter().lat() : 0
  ...
  <div className="my-map-container">
    <GoogleMap
      ...
      onBoundsChanged={() => {
        props.onBoundsChanged(props.currentCenter)
      }}
...
export default class MapWrapper extends React.Component {
  ...
  handleClickResetPin = () => {
    if (this.map) {
      ...
      this.props.onChange({
        action: 'reset',
        value: { lat: this.props.lastSearched.lat, lng: this.props.lastSearched.lng },
      })
      this.setState({
        currentCenter: {
          action: 'reset',
          ...
          <div className="map-label">
            ...
            <If condition={!disabled}>
              <span
                role="button"
                tabIndex="0"
                onClick={this.handleClickResetPin}
                className="reset-pin-btn"
              >
                {t('cms.properties.edit.address.reset_pin')}
              </span>
            </If>
          ...
          <Row>
            <Col span={12}>
              <span className="address__map__coordinate--lng">
                ...
                {this.state.currentCenter.value.lng && this.state.currentCenter.value.lng !== null
            ...
            <Col span={12}>
              <span className="address__map__coordinate--lat">
                ...
                {this.state.currentCenter.value.lat && this.state.currentCenter.value.lat !== null
