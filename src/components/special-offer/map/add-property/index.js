...
class AddProperty extends React.Component {
  ...
  onAddProperty = () => {
    ...
    const unitTypeIds = this.state.unitTypeIds.filter((item, index) =>
      (!this.state.listingIds[index]));
    const params = {
      id: this.props.offerInfo.id,
      propertyIds: this.state.unitTypeIds.length === 0 ? [this.state.selectProperty] : null,
      unitTypeIds,
      listingIds: this.state.listingIds.length > 0 ?
        Array.prototype.concat.apply([], listingIds) : null,
    };
  ...
  onEditProperty = () => {
    ...
    const newUnitTypeIds = this.state.unitTypeIds.filter((item, index) =>
      (!this.state.listingIds[index] || this.state.listingIds[index].length === 0));
    let oldListingIds = this.props.initialParams.listingIds.filter(d => d);
    oldListingIds = Array.prototype.concat.apply([], oldListingIds);
    const oldUnitTypeIds = this.props.initialParams.unitTypeIds.filter((item, index) =>
  ...
  renderUnitType = () => {
    ...
    const unitTypeComponents = this.state.unitTypeIds.map((item, index) => {
      ...
      return (<Row key={ item } className="add-property__unit-type">
        <Col span={ 12 }>
          <div
          ...
          >
            <Select
              ...
              value={ item }
              onSelect={ (value) => { this.handleUnitTypeSelect(value, index); } }
              getPopupContainer={ () => this.unitTypeSelector || document.body }
            >
              <For each="d" of={ this.state.unitType } >
                <Option key={ d.id } value={ d.id } disabled={ isDisabled(d) } >{d.name}</Option>
              </For>
            </Select>
          </div>
          <Icon type="minus-circle" className="add-property__icon" onClick={ () => { this.removeUnitType(index); } } />
          <If condition={ noListings(item) }>
            <Tooltip placement="rightTop" title={ this.props.t('cms.special_offer.map.no_listing') }>
              <Icon className="add-property__icon-red" type="warning" />
            </Tooltip>
          </If>
        </Col>
        <If condition={ (listings.length === 1 && listings[0].type !== 'PLACEHOLDER') || listings.length > 1 }>
          <Col span={ 12 }>
            <div
            ...
            >
              <Select
                ...
                optionFilterProp="children"
                onChange={ (value) => { this.handleListingSelect(value, index); } }
  ...
  formRender = (children) => {
    ...
    return (
      <div>
        ...
        <Button disabled={ !this.state.selectProperty } type="primary" className="create-offer-content__continue-btn" onClick={ this.onAddProperty } >
          {this.props.t('cms.sepcial_offer.add.btn')}
        </Button>
      </div>
    );
