  ...
  opreatProperty = (record) => {
    let params = {
      ...
      unitType: record.property.unitTypes.edges ?
        this.formatUnitTypes(record.property.unitTypes) : record.property.unitTypes,
      listingIds: [],
      unitTypeIds: [],
    };

    params.unitType.forEach((unitTypeItem) => {
      const listings = record.listing.filter(listItem =>
        (listItem.unitType.id === unitTypeItem.id));
    ...
    params.unitTypeIds.forEach((linkId) => {
      unitTypes = record.unitType.filter(linkUnitTypeItem =>
        (linkUnitTypeItem.id !== linkId));
    });
      ...
      render: (text, record) => {
        ...
        return (<div>
          ...
          <If condition={ record.children.length > 1 }>
            <span>
              {`${text.length} ${this.props.t('cms.special_offer.form.unit_types')}`}
            </span>
          </If>
        </div>);
      },
      ...
      render: (text, record) => {
        ...
        if (text.length > 0) {
          return (<div>
            ...
            <If condition={ record.children.length !== 1 }>
              <span>
                {`${text.length} ${this.props.t('cms.special_offer.form.listings')}`}
              </span>
            </If>
          </div>);
        }
      ...
      render: (text, record) => (<div>
        <If condition={ Object.prototype.toString.call(record.listing) !== '[object String]' }>
          <a
            role="button"
            tabIndex="0"
            onClick={ () => this.opreatProperty(record) }
          >
            <Icon type="edit" />
          </a>
        ...
        <Popconfirm
          ...
          onConfirm={ () => { this.unlinkOffer(record); } }
          placement="topRight"
    ...
    allProperties.forEach((property, index) => {
      ...
      this.props.offerInfo.unitTypes.forEach((unitType) => {
        if (unitType.property.id === property.id) {
          ...
          data = update(data, { [index]: { children: { $push: [{
            key: unitType.id,
            listing: '',
      ...
      <div>
        <div className="special-offer__map-property-header">
          <Button onClick={ this.addProperty }>
            <Icon type="plus" />
            {this.props.t(
              'cms.table.header.button.add_new_property',
            )}
          </Button>
        </div>
        <div className="table-list__wrapper">
          <Table
            ...
            indentSize={ 0 }
            loading={
              this.props.communication.link.status ===
        ...
        >
          <Button type="primary" className="create-offer-content__continue-save-btn" onClick={ this.props.goToNextStep } >
