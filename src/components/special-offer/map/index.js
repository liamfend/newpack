...
export default class Map extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      offerMapType: this.props.offerInfo.cities &&
        this.props.offerInfo.cities.length > 0 ?
        mapType.city : mapType.property,
      initialParams: {
        search: '',
        selectProperty: '',
        unitType: [],
  ...
  componentWillReceiveProps(nextProps) {
    this.operationChecker(
      nextProps,
      'link',
      this.props.t('cms.message.offers.linked_success'),
    );
    this.operationChecker(
      nextProps,
      'unlink',
      this.props.t('cms.message.offers.unlinked_success'),
    );
  }
  ...
  render() {
    ...
    const cityDisable = (offerInfo.properties && offerInfo.properties.length > 0) ||
    ...
    return (<div className="special-offer__map">
      <Radio.Group onChange={ this.handleTypeChange } value={ this.state.offerMapType } className="special-offer__map-group">
        <Tooltip placement="top" title={ propertyDisable && this.props.t('cms.special_offer.map.property.tooltip') }>
          <Radio.Button
        ...
        <Tooltip placement="right" title={ cityDisable && this.props.t('cms.special_offer.map.property.tooltip') }>
          <Radio.Button
      ...
      <If
        condition={ this.state.offerMapType === mapType.property &&
