  ...
  onContinue = () => {
    this.props.form.validateFields((err, values) => {
      if (!err) {
        ...
        const endDateValue =
          values.endDate.hasEndDate === endDate.yes
            ? moment(values.endDate.endDateValue).format('YYYY-MM-DD')
            : null
        ...
        message.loading(
          this.props.t(
            `cms.message.special_offer.${this.props.isCreateOffer ? 'creating' : 'updating'}`,
          ),
          1,
        )
      }
          ...
          <Form onSubmit={this.handleSubmit} layout="vertical">
                ...
                rules: [
                  {
                    required: true,
                    type: 'object',
                    message: this.props.t('cms.special_offer.error.form.start_date'),
                  },
                ],
              })(
                <DatePicker
            ...
            <FormItem label={t('cms.special_offer.form.end_date')}>
              {getFieldDecorator('endDate', {
                initialValue: {
                  endDateValue: offerInfo.endDate ? moment(offerInfo.endDate) : undefined,
                  hasEndDate: offerInfo.endDate ? endDate.yes : endDate.no,
                },
              })(<EndDatePicker startDate={getFieldValue('startDate')} t={t} />)}
            </FormItem>
            <FormItem label={t('cms.special_offer.form.exclusive_title')}>
              {getFieldDecorator('exclusive', {
                initialValue: offerInfo.exclusive
                  ? offerCategory.exclusive
                  : offerCategory.nonExclusize,
              })(
            ...
            >
              {getFieldDecorator('displaySetting', {
                initialValue:
                  offerInfo && offerInfo.ipAddresses && offerInfo.ipAddresses.length > 0
                    ? displaySetting.accordingIPAddress
            ...
            <FormItem style={{ height: 30, marginBottom: 24 }}>
              <If condition={getFieldValue('displaySetting') === displaySetting.accordingIPAddress}>
                {getFieldDecorator('ipAddresses', {
                  initialValue: offerInfo && offerInfo.ipAddresses ? offerInfo.ipAddresses : [],
                  rules: [
                    {
                      required: true,
