...
const useAvailabilityAndPrice = ({ form, showPlaceHolder }) => {
  const handleDiscountValueChange = (inputName, value) => {
    ...
    if (inputName === 'discountValue' && value) {
      ...
      discountValue = form.getFieldValue('discountValue') > 0 ?
  ...
  const discountValueParameter = useMemo(() => ({
    ...
    parser: value => (form.getFieldValue('discountType') === 'PERCENTAGE' ? value.replace('%', '') : value),
    onChange: (value) => { handleDiscountValueChange('discountValue', value); },
