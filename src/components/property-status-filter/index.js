...
export default class PropertyStatusFilter extends React.Component {
  ...
  getStatusFilterList = () => ((isLandlordRole() || isContentSpecialistRole()) ?
    this.statusListWithOutNew : this.statusList);
    ...
    return (
      <div className="property-status-filter">
        <RadioGroup defaultValue="all" onChange={ this.handleChangeStatus }>
          <For each="status" of={ this.getStatusFilterList() }>
            <RadioButton
              value={ status }
              key={ status }
              checked={ filters.statuses === status.toUpperCase().replace('-', '_')
        ...
        <div className="property-status-filter__filter-container">
          ...
          <If condition={ (!type || ['citySlug', 'countrySlug'].indexOf(type) === -1) && !isLandlordRole() }>
            <div className="property-status-filter__country-filter">
              <Dropdown
                overlay={ this.getCountryMenu() }
              >
                <a className="ant-dropdown-link">
                  <Choose>
                    <When condition={
                      this.popularCountriesList.find(item => item === filters.countrySlug) }
                    >
                      <span >{ this.props.t(`cms.property_status_filter.countries_filter.label.${filters.countrySlug}`) }</span>
                    </When>
                    <Otherwise>
                      { this.props.t('cms.property_status_filter.countries_filter.title.filter_your_country') }
                    </Otherwise>
                  </Choose>
                  <Icon type="down" />
                </a>
          ...
          <div className="property-status-filter__sort-filter">
            <Dropdown
              overlay={ this.getSortMenu() }
            >
              <a className="ant-dropdown-link">
                <Choose>
                  <When condition={
                    this.sortList.find(item => item === filters.sort) }
                  >
                    <span >{ this.props.t(`cms.property_status_filter.sort_filter.label.${filters.sort}`) }</span>
                  </When>
                  <Otherwise>
