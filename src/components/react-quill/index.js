...
export default class ReactQuillWrapper extends React.Component {
  ...
  onChange = (content, delta, source) => {
    if (source === 'user') {
      this.props.onChange(this.changeRichText(content))
    }
  }

  onBlur = (range, source, editor) => {
    if (this.props.onBlur) {
      this.props.onBlur(this.changeRichText(editor.getHTML()))
    }
  }

  formats = ['header', 'bold', 'italic', 'underline', 'link', 'list', 'bullet']
