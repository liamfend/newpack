  ...
  return class extends Component {
    ...
    handleShiftSelect = (e, node) => {
      if (this.lastClickedNode && e.shiftKey) {
        ...
        if (elements[lastSelectedIndex].isSelected() &&
          !elements[currentIndex].isSelected()) {
          let start;
          let end;
    ...
    canDrop = (sourceContainer) => {
      ...
      return (this.props.multiple ||
        (selected.length === 1 && this.state.items.length === 0));
    }
    ...
    initDragData = () => {
      ...
      if (elements.length) {
        ...
        for (let i = 0; i <= virtualLen; i++) {
          const containerRightX = (containerBoundingRect.x + containerBoundingRect.width)
    ...
    handleMove = (e) => {
      if (this.dragInitialEvent) {
        ...
        if ((distance < moveX || distance < moveY) && !this.dragging) {
          ...
          // Find the node of sortable element
          const node = closest(
            this.dragInitialEvent.target,
            el => this.manager.getElementByNode(el),
          );
    ...
    handleSortEnd = (e, sourceContainer) => {
      ...
      if (
        this.dragData.coveringIndex !== -1 &&
        this.canDrop(sourceContainer)
      ) {
    ...
    setContainerStyle(sourceContainer) {
      ...
      const hoveringContainer = (x <= movingCoordinate.x && movingCoordinate.x <= x + width) &&
        (y <= movingCoordinate.y && movingCoordinate.y <= y + height);
      ...
      if (virtualList.length) {
        ...
        const containerHeight = ((lastVirtualElement.y + virtualHeight) - virtualInitialAxis.y)
    ...
    checkCoveringIndex = () => {
      ...
      virtualList.some((virtualElement, index) => {
        ...
        const elementYBottom = virtualElement.y + offsetAxis.y +
           tolerateEdge + virtualElement.height;
    ...
    animateNodes() {
      ...
      this.dragData.actualList.forEach((actualElement, index) => {
        ...
        if (this.dragData.coveringIndex !== -1 &&
            this.dragData.coveringIndex <= shouldPlaceIndex) {
          shouldPlaceIndex += 1;
        }
        ...
        elements[index].node.style[`${vendorPrefix}Transform`] = `translate3d(${
          ...
          shouldPlaceElement.y - actualElement.y
        }px,0)`;
      });
    }
