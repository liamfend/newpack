...
export default class DragLayer {
  ...
  createHelper(list, e) {
    ...
    if (
      (e.clientY < top || (top + this.height) < e.clientY) ||
      (e.clientX < left || (left + this.width) < e.clientX)
    ) {
    ...
    if (this.axis.x) {
      this.minTranslate.x =
        -this.boundingClientRect.left - (this.width / 2);
      this.maxTranslate.x =
    ...
    if (this.axis.y) {
      this.minTranslate.y =
        -this.boundingClientRect.top - (this.height / 2);
      this.maxTranslate.y =
