  ...
  renderWarning = () => (
    <div className="edit-layout__warning">
      ...
      <div className="edit-layout__warning__btn-wrapper">
        <Button
          className="edit-layout__warning__btn"
          onClick={() => {
            this.toogleWarning(false)
          }}
          type="primary"
        >
  ...
  render() {
    ...
    return (
      <div className={classNames('edit-layout', this.props.className)}>
        <LeaveAlert history={this.props.history} t={this.props.t} when={this.hasUnsavedData()} />
        <div className="edit-layout__header-container">
          <div className="edit-layout__summary-container">
            ...
            <span
              className={classNames('edit-layout__summary-status', {
                'edit-layout__summary-status--green': this.props.published,
              })}
            >
              {this.props.t(
                `cms.edit.layout.status.${this.props.published ? 'published' : 'unpublish'}`,
              )}
            </span>
          </div>
          <div className="edit-layout__action-container">
            <Link
              to={generatePath(this.getCloseUrlType(), {}, this.getFilters())}
              className="edit-layout____link"
            >
              <button
                type="button"
                className="edit-layout__action-btn edit-layout__action-btn--close"
                onClick={this.handleClose}
              >
                {this.props.t('cms.edit.layout.status.btn.close')}
              </button>
            </Link>
            <If condition={!this.props.published}>
              <button
                type="button"
                className="edit-layout__action-btn edit-layout__action-btn--save"
                onClick={() => {
                  this.handleSave()
                }}
              >
                {this.props.t('cms.edit.layout.status.btn.save')}
              </button>
            </If>
            <Popover
            ...
            >
              <button
                type="button"
                className="edit-layout__action-btn edit-layout__action-btn--review"
                onClick={this.handleReview}
              >
                {this.props.t('cms.edit.layout.status.btn.review')}
              </button>
            </Popover>
          </div>
        </div>
