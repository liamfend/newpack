...
export default class OthersFacilities extends React.Component {
  ...
  componentDidMount() {
    this.othersFacilitiesInput.addEventListener('keyup', (e) => {
      if (e.keyCode === 13) {
        if (this.state.facilities.indexOf(this.state.value) === -1 &&
  ...
  showFacilityLabel = (facility) => {
    const label = this.props.findPropertyFacilityBySlug(facility, 'others') ?
      this.props.findPropertyFacilityBySlug(facility, 'others').label : facility;
    ...
    return (
      <div>
        <div
          ...
          ref={ (node) => { this.othersFacilitiesContainer = node; } }
        >
          <div className="others-facilities__container">
            ...
            <input
              ...
              ref={ (node) => { this.othersFacilitiesInput = node; } }
        ...
        <div >
          <span className="others-facilities__error-msg">
            {isInputError ? this.props.t('cms.properties.edit.facilities.other.length_limit_error') : ''}
          </span>
          <span className={ classNames('others-facilities__counter', {
